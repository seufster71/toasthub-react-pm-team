...
class PMTeamContainer extends Component {
	...
	onUserRoleModify = (item) => {
		fuLogger.log({level:'TRACE',loc:'PMTeamContainer::onUserRoleModify',msg:"test"+item.id});
	...
	onUserRoleSave = () => {
		fuLogger.log({level:'TRACE',loc:'PMTeamContainer::onUserRoleSave',msg:"test"});
		let errors = utils.validateFormFields(this.props.pmteam.prefForms.ADMIN_USER_ROLE_FORM,this.props.pmteam.inputFields, this.props.appPrefs.prefGlobal.LANGUAGES);
		
		if (errors.isValid){
			let searchCriteria = {'searchValue':this.state['PM_TEAM_SEARCH_input'],'searchColumn':'PM_TEAM_TABLE_NAME'};
			this.props.actions.saveRolePermission({state:this.props.pmteam});
		} else {
			this.setState({errors:errors.errorMap});
		}
	}
